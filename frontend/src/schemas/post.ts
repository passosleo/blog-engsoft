...
export const createPostSchema = z.object({
  ...
  content: z.string({ required_error: isRequired })
    .min(3, validations.string.isMinThenThree)
    .refine((value) => {
      if (value === '<p><br></p>' || value === '<h1><br></h1>' || value === '<h2><br></h2>' || value === '<h3><br></h3>') {
        return false;
      }
      return true;
    }, { message: isRequired })
    .refine((value) => validarConteudo(value), { message: "O conteúdo deve ter mais que 3 caracteres" }),
  categoryId: z.string({ required_error: isRequired }).uuid({ message: isRequired }),
